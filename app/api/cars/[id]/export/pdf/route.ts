...
export async function GET(
  ...
  const html = `
    ...
    <html>
    <head>
      <title>${car.make} ${car.model} - Vehicle History</title>
      <style>
    ...
    <body>
      <div class="cover">
        ...
        <p class="subtitle">Complete Vehicle History & Documentation</p>
        <p style="color: #FF6B35; font-weight: bold;">${car.year || 'N/A'}</p>
      </div>

      <div class="specs">
        <div class="spec-item">
          <div class="spec-label">Engine</div>
          <div class="spec-value">${car.engineCode || 'N/A'}</div>
        </div>
        <div class="spec-item">
          <div class="spec-label">Factory HP</div>
          <div class="spec-value">${car.factoryHp || 'N/A'} hp</div>
        </div>
        <div class="spec-item">
          <div class="spec-label">Project Goal</div>
      ...
      <div class="section">
        <h2>TÜV Summary</h2>
        <p>Modifications breakdown by registration status:</p>
      ...
      <div class="footer">
        <p>Generated by DRIVETUNING</p>
        <p>Document created on ${new Date().toLocaleDateString('de-DE')}</p>
      </div>
    </body>
    </html>
  `
  ...
  return new NextResponse(html, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="${car.make}-${car.model}-history.html"`,
    },
  })
}
